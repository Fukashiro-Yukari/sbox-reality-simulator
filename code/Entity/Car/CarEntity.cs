...
public partial class CarEntity : Prop, IUse
{
	...
	protected override void OnDestroy()
	{
		...
		if ( driver is RealityPlayer player )
		{
			RemoveDriver( player );
		}
	}
	...
	protected void Tick()
	{
		if ( driver is RealityPlayer player )
		{
			if ( player.LifeState != LifeState.Alive || player.Vehicle != this || player.Ragdoll.IsValid() )
			{
				RemoveDriver( player );
			}
		}
	...
	public override void Simulate( Client owner )
	{
		...
		using ( Prediction.Off() )
		{
			...
			{
				if ( owner.Pawn is RealityPlayer player && !player.IsUseDisabled() )
				{
					RemoveDriver( player );
	...
	private void RemoveDriver( RealityPlayer player )
	{
		driver = null;
	...
	{
		if ( user is RealityPlayer player && player.Vehicle == null && timeSinceDriverLeft > 1.0f )
		{
			player.Vehicle = this;
	...
	public override void StartTouch( Entity other )
	{
		...
		if ( other is RealityPlayer player && player.Vehicle == null )
		{
	...
	protected override void OnPhysicsCollision( CollisionEventData eventData )
	{
		...
		if ( eventData.Entity is RealityPlayer player && player.Vehicle != null )
		{
			return;
		}
		...
		if ( speed > minImpactSpeed )
		{
			if ( eventData.Entity.IsValid() && eventData.Entity != this )
			{
				...
				if ( eventData.Entity.LifeState == LifeState.Dead && eventData.Entity is not RealityPlayer )
				{
		...
		if ( speed > 1000 && driver.IsValid() && driver is RealityPlayer ply )
		{
			ply.BecomeRagdoll();
			RemoveDriver( ply );
		}
